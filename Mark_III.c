...
int solvePuzzle(){
	...
	//printf("word1 len : %d \n", word1_length);
	//printf("word2 len : %d \n", word2_length);
	//printf("grid grid_len %d\n",grid_len);
	int x;
	//printf("%d %d\n",puzzle_count,word_count);
	if ((puzzle_count == word_count)&&(puzzle_count==0)){
		return -1;
	}
	for ( x = 0; x < 10; x++){
		if (grid[x] == 0){
		...
		for (i = 0; i < 10; ++i){
				...
				if (streakByRow((char *)puzzle_Array[i],grid[i]) == 1){
					...
					for (j = 0; j < strlen(word_Array[result_Array_Row_Str[1]-1]) ; j++){
						...
						result_Array[i][j+result_Array_Row_Str[0]] = word_Array[result_Array_Row_Str[1]-1][j];
					}
					word_counter++;
					//continue;
				}else{
		...
		for (i = 0; i < grid_len; ++i){
			for (j = 0; j < grid[i]; j++){
				...
				if (puzzle_Array[j][i] == '\n' || puzzle_Array[j][i] == '\0'){
					...
					temp_Array[j] = puzzle_Array[j][i];
				}
			}
			//printf("%s\n", temp_Array);
			if (streakByColumn(temp_Array,grid_len) == 1){
				//printf("streak found!!!\n");
				//printf("i %d j %d %s\n",i,j, word_Array[result_Array_Column_Str[1]-1]);
				for (j = 0; j < strlen(word_Array[result_Array_Column_Str[1]-1]) ; j++){
					// Replace the hash with the letter 
					printf("Replace\n");
					result_Array[j][i] = word_Array[result_Array_Column_Str[1]-1][j];
				}
				word_counter++;
				break;
			}else{
