		...
		if (column_Counters[i] == 0){
			...
			for (j = 0; j < column_Counters[j]; j++){
				...
				if (column_Counters[i] == (int)strlen(word_Array[j])){
					printf("Match word length\n");
					for (x = 0; x < strlen(word_Array[j]); x++){
						if ( (tolower(grid_Column_Array[x+column_Starts[i]][i]) == tolower(word_Array[j][x])) || (tolower(grid_Column_Array[x+column_Starts[i]][i]) == '#') ){
							result_Array[x+column_Starts[i]][i] = word_Array[j][x];
							printf("i %d j %d",i,(x+column_Starts[i]));
							continue;
						}else{
